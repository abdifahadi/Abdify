window.AbdiEngine = {
	Player: {
		addEventListener: (type, callback) => {
			if (!(type in AbdiEngine.Player.eventListeners)) {
				AbdiEngine.Player.eventListeners[type] = [];
			}
			AbdiEngine.Player.eventListeners[type].push(callback);
		},
		dispatchEvent: (event) => {
			if (!(event.type in AbdiEngine.Player.eventListeners)) {
				return true;
			}
			const stack = AbdiEngine.Player.eventListeners[event.type];
		...
		eventListeners: {},
		seek: (p) => {
			const duration = p <= 1 ? Math.round(p * AbdiEngine.Player.origin._state.duration) : p;
			AbdiEngine.Player.origin.seekTo(duration);
		},
		getProgress: () =>
			(AbdiEngine.Player.origin._state.isPaused ? 0 : Date.now() - AbdiEngine.Player.origin._state.timestamp) +
			AbdiEngine.Player.origin._state.positionAsOfTimestamp,
		getProgressPercent: () => AbdiEngine.Player.getProgress() / AbdiEngine.Player.origin._state.duration,
		getDuration: () => AbdiEngine.Player.origin._state.duration,
		setVolume: (v) => {
			AbdiEngine.Platform.PlaybackAPI.setVolume(v);
		},
		increaseVolume: () => {
			AbdiEngine.Platform.PlaybackAPI.raiseVolume();
		},
		decreaseVolume: () => {
			AbdiEngine.Platform.PlaybackAPI.lowerVolume();
		},
		getVolume: () => AbdiEngine.Platform.PlaybackAPI._volume,
		next: () => {
			AbdiEngine.Player.origin.skipToNext();
		},
		back: () => {
			AbdiEngine.Player.origin.skipToPrevious();
		},
		togglePlay: () => {
			AbdiEngine.Player.isPlaying() ? AbdiEngine.Player.pause() : AbdiEngine.Player.play();
		},
		isPlaying: () => !AbdiEngine.Player.origin._state.isPaused,
		toggleShuffle: () => {
			AbdiEngine.Player.origin.setShuffle(!AbdiEngine.Player.origin._state.shuffle);
		},
		getShuffle: () => AbdiEngine.Player.origin._state.shuffle,
		setShuffle: (b) => {
			AbdiEngine.Player.origin.setShuffle(b);
		},
		toggleRepeat: () => {
			AbdiEngine.Player.origin.setRepeat((AbdiEngine.Player.origin._state.repeat + 1) % 3);
		},
		getRepeat: () => AbdiEngine.Player.origin._state.repeat,
		setRepeat: (r) => {
			AbdiEngine.Player.origin.setRepeat(r);
		},
		getMute: () => AbdiEngine.Player.getVolume() === 0,
		toggleMute: () => {
			AbdiEngine.Player.setMute(!AbdiEngine.Player.getMute());
		},
		setMute: (b) => {
			if (b !== AbdiEngine.Player.getMute()) {
		...
		getHeart: () => AbdiEngine.Player.origin._state.item.metadata["collection.in_collection"] === "true",
		pause: () => {
			AbdiEngine.Player.origin.pause();
		},
		play: () => {
			AbdiEngine.Player.origin.resume();
		},
		playUri: async (uri, context = {}, options = {}) => {
			return await AbdiEngine.Player.origin.play({ uri: uri }, context, options);
		},
		removeEventListener: (type, callback) => {
			if (!(type in AbdiEngine.Player.eventListeners)) return;
			const stack = AbdiEngine.Player.eventListeners[type];
			for (let i = 0; i < stack.length; i++) {
				if (stack[i] === callback) {
					stack.splice(i, 1);
					return;
		...
		skipBack: (amount = 15e3) => {
			AbdiEngine.Player.origin.seekBackward(amount);
		},
		skipForward: (amount = 15e3) => {
			AbdiEngine.Player.origin.seekForward(amount);
		},
		setHeart: (b) => {
			const uris = [AbdiEngine.Player.origin._state.item.uri];
			if (b) {
				AbdiEngine.Platform.LibraryAPI.add({ uris });
			} else {
				AbdiEngine.Platform.LibraryAPI.remove({ uris });
			}
		},
		toggleHeart: () => {
			AbdiEngine.Player.setHeart(!AbdiEngine.Player.getHeart());
		},
	},
	test: () => {
		function checkObject(object) {
			...
			for (const method of methods) {
				if (objectToCheck[method] === undefined || objectToCheck[method] === null) {
					console.error(`${name}.${method} is not available. Please open an issue in the AbdiEngine repository to inform us about it.`);
					count--;
				}
			}
		...
		const objectsToCheck = new Set([
			...
			{
				objectToCheck: AbdiEngine.Player,
				name: "AbdiEngine.Player",
				methods: new Set([
					"addEventListener",
					"back",
					"data",
			...
			{
				objectToCheck: AbdiEngine.ReactComponent,
				name: "AbdiEngine.ReactComponent",
				methods: new Set([
					"RightClickMenu",
					"ContextMenu",
					"Menu",
			...
			{
				objectToCheck: AbdiEngine.ReactComponent.Cards,
				name: "AbdiEngine.ReactComponent.Cards",
				methods: new Set([
					"Default",
					"Hero",
					"CardImage",
			...
			{
				objectToCheck: AbdiEngine.ReactHook,
				name: "AbdiEngine.ReactHook",
...
(function waitForPlatform() {
	if (!AbdiEngine._platform) {
		setTimeout(waitForPlatform, 50);
		return;
	}
	const { _platform } = AbdiEngine;
	for (const key of Object.keys(_platform)) {
		if (key.startsWith("get") && typeof _platform[key] === "function") {
			AbdiEngine.Platform[key.slice(3)] = _platform[key]();
		} else {
			AbdiEngine.Platform[key] = _platform[key];
		}
	}
})();

(function addMissingPlatformAPIs() {
	if (!AbdiEngine.Platform?.version && !AbdiEngine.Platform?.Registry) {
		setTimeout(addMissingPlatformAPIs, 50);
		return;
	}
	const os = AbdiEngine.Platform.operatingSystem;
	const version = AbdiEngine.Platform.version.split(".").map((i) => Number.parseInt(i, 10));
	if (version[0] === 1 && version[1] === 2 && version[2] < 38) return;

	for (const [key, _] of AbdiEngine.Platform.Registry._map.entries()) {
		...
		if (Object.hasOwn(AbdiEngine.Platform, symbolName)) continue;
		try {
			const resolvedAPI = AbdiEngine.Platform.Registry.resolve(key);
			AbdiEngine.Platform[symbolName] = resolvedAPI;

			console.debug(`[AbdiEngineWrapper] Resolved PlatformAPI from Registry: ${symbolName}`);
		} catch (err) {
			console.error(`[AbdiEngineWrapper] Error resolving PlatformAPI from Registry: ${symbolName}`, err);
		}
	}
})();
...
function applyScrollingFix() {
	if (!AbdiEngine.Platform?.version) {
...
(async function addProxyCosmos() {
	if (!AbdiEngine.Player.origin?._cosmos && !AbdiEngine.Platform?.Registry) {
		setTimeout(addProxyCosmos, 50);
		return;
	}

	const _cosmos = AbdiEngine.Player.origin?._cosmos ?? AbdiEngine.Platform?.Registry.resolve(Symbol.for("Cosmos"));
	...
	const handler = {
		get: (target, prop, receiver) => {
			...
			const version = AbdiEngine.Platform.version.split(".").map((i) => Number.parseInt(i));
			if (version[1] >= 2 && version[2] < 31) return internalFetch;

			return async function (url, body) {
				const urlObj = new URL(url);

				const corsProxyURLTemplate = window.localStorage.getItem("AbdiEngine:corsProxyTemplate") ?? "https://cors-proxy.AbdiEngine.app/{url}";
				...
				if (shouldUseCORSProxy) {
					...
					try {
						...
						console.error("[AbdiEngineWrapper] Invalid CORS Proxy URL template");
					}
				}

				const Authorization = `Bearer ${AbdiEngine.Platform.AuthorizationAPI.getState().token.accessToken}`;
				let injectedHeaders = {};
				if (isWebAPI) injectedHeaders = { Authorization };
				if (isSpClientAPI) {
					injectedHeaders = {
						Authorization,
						"Spotify-App-Version": AbdiEngine.Platform.version,
						"App-Platform": AbdiEngine.Platform.PlatformData.app_platform,
					};
				}
	...
	while (!AbdiEngine.Player.origin) await new Promise((r) => setTimeout(r, 50));
	AbdiEngine.Player.origin._cosmos = new Proxy(_cosmos, handler);
	Object.defineProperty(AbdiEngine, "CosmosAsync", {
		get: () => {
			return AbdiEngine.Player.origin?._cosmos;
		},
	});
})();
...
(async function hotloadWebpackModules() {
	...
	console.log("[AbdiEngineWrapper] Waiting for required webpack modules to load");
	...
	AbdiEngine.React = cache.find((m) => m?.useMemo);

	while (!webpackDidCallback) {
		await new Promise((r) => setTimeout(r, 100));
	}
	console.log("[AbdiEngineWrapper] All required webpack modules loaded");
	chunks = Object.entries(require.m);
	cache = Object.keys(require.m).map((id) => require(id));
	AbdiEngine.Events.platformLoaded.fire();
	...
	const _ScrollableContainer = (() => {
		...
		function injectStyles() {
			...
			style.className = "AbdiEngine-scrollable-container";
			style.textContent = `
.AbdiEngine-sc-contentArea { overflow: hidden; position: relative; }
.AbdiEngine-sc-scroller { display: flex; align-items: center; overflow-x: auto; scrollbar-width: none; white-space: nowrap; width: 100%; -ms-overflow-style: none; overscroll-behavior-x: contain; will-change: transform; }
@media (prefers-reduced-motion: no-preference) { .AbdiEngine-sc-scroller { scroll-behavior: smooth; } }
.AbdiEngine-sc-scroller::-webkit-scrollbar { display: none; }
.AbdiEngine-sc-scroller.AbdiEngine-sc-snap { scroll-snap-type: inline mandatory; }
.AbdiEngine-sc-scroller.AbdiEngine-sc-snap .AbdiEngine-sc-snapCenter [data-carousel-item] { scroll-snap-align: center; }
.AbdiEngine-sc-scroller.AbdiEngine-sc-snap .AbdiEngine-sc-snapStart [data-carousel-item] { scroll-snap-align: start; }
.AbdiEngine-sc-scroller.AbdiEngine-sc-wheelEnabled { overscroll-behavior: contain; }
.AbdiEngine-sc-scroller.AbdiEngine-sc-maskGradient { --sc-start-color: #000; --sc-end-color: #000; -webkit-mask-composite: source-in, xor; mask-composite: intersect; -webkit-mask-image: linear-gradient(90deg, var(--sc-start-color) 0, #000 120px), linear-gradient(90deg, #000 calc(100% - 120px), var(--sc-end-color) 100%); mask-image: linear-gradient(90deg, var(--sc-start-color) 0, #000 120px), linear-gradient(90deg, #000 calc(100% - 120px), var(--sc-end-color) 100%); -webkit-mask-size: 100% 100%; mask-size: 100% 100%; }
.AbdiEngine-sc-scroller.AbdiEngine-sc-maskGradient.AbdiEngine-sc-maskStart { --sc-start-color: transparent; }
.AbdiEngine-sc-scroller.AbdiEngine-sc-maskGradient.AbdiEngine-sc-maskEnd { --sc-end-color: transparent; }
.AbdiEngine-sc-linearGradient::before, .AbdiEngine-sc-linearGradient::after { bottom: 0; content: ""; height: 100%; opacity: 0; pointer-events: none; position: absolute; top: 0; transition: opacity .15s ease-out; width: 120px; z-index: 2; }
.AbdiEngine-sc-linearGradient::before { background: linear-gradient(90deg, var(--carousel-start-chevron-gradient, var(--spice-main)) 0, transparent 100%); inset-inline-start: 0; }
.AbdiEngine-sc-linearGradient::after { background: linear-gradient(-90deg, var(--carousel-end-chevron-gradient, var(--spice-main)) 0, transparent 100%); inset-inline-end: 0; }
.AbdiEngine-sc-linearGradient.AbdiEngine-sc-lgStart::before { opacity: 1; }
.AbdiEngine-sc-linearGradient.AbdiEngine-sc-lgEnd::after { opacity: 1; }
.AbdiEngine-sc-carousel { bottom: 0; left: 0; position: absolute; right: 0; top: 0; justify-content: space-between; align-items: center; display: flex; pointer-events: none; }
.AbdiEngine-sc-chevronBtn { display: flex; border: none; border-radius: 50%; cursor: pointer; justify-content: center; align-items: center; backdrop-filter: var(--chevrons-button-backdrop-filter, none); background: transparent; background-color: var(--chevrons-button-color, var(--background-elevated-base)); height: 24px; opacity: 0; position: relative; transition: color .15s ease-out, opacity .15s ease-out, background-color .15s ease-out, translate .15s ease-out; translate: 0; width: 24px; z-index: 3; pointer-events: none; color: var(--text-base, #fff); }
.AbdiEngine-sc-chevronBtn > * { opacity: .7; z-index: 2; }
.AbdiEngine-sc-chevronBtn:hover { background-color: var(--chevrons-button-hover-color, var(--background-elevated-highlight)); }
.AbdiEngine-sc-chevronBtn:hover > * { opacity: 1; }
.AbdiEngine-sc-chevronBtn.AbdiEngine-sc-chevronVisible { opacity: 1; pointer-events: auto; }
.AbdiEngine-sc-onHover .AbdiEngine-sc-chevronBtn { opacity: 0; }
.AbdiEngine-sc-contentArea:hover .AbdiEngine-sc-onHover .AbdiEngine-sc-chevronBtn.AbdiEngine-sc-chevronVisible { opacity: 1; }
.AbdiEngine-sc-contentArea:hover .AbdiEngine-sc-onHover .AbdiEngine-sc-chevronStart.AbdiEngine-sc-chevronVisible { translate: 8px; }
.AbdiEngine-sc-contentArea:hover .AbdiEngine-sc-onHover .AbdiEngine-sc-chevronEnd.AbdiEngine-sc-chevronVisible { translate: -8px; }
body[data-dragging-uri-type] .AbdiEngine-sc-chevronBtn { pointer-events: none; }`;
			document.head.appendChild(style);
		}

		function useDragToScroll({ isDisabled = true } = {}) {
			const { useRef, useCallback } = AbdiEngine.React;
		...
		function useScrollState(scrollerRef, contentRef) {
			const { useState, useCallback, useEffect } = AbdiEngine.React;
		...
		function ScrollableContainerComponent(props) {
			const { useRef, useCallback, useMemo } = AbdiEngine.React;
			const h = AbdiEngine.ReactJSX.jsx;
			const hsf = AbdiEngine.ReactJSX.jsxs;
			const cn = AbdiEngine.classnames;
			...
			const makeChevron = (svgPath, position, visible, dir) =>
				h("div", {
					className: cn("AbdiEngine-sc-chevronBtn", `AbdiEngine-sc-chevron${position}`, { "AbdiEngine-sc-chevronVisible": visible }),
					onClick: (e) => {
						e.preventDefault();
						e.stopPropagation();
						navigate(dir);
			...
			return hsf("div", {
				className: cn("AbdiEngine-sc-contentArea", className, {
					"AbdiEngine-sc-linearGradient": isLinearGradient,
					"AbdiEngine-sc-lgStart": isLinearGradient && canGoStart,
					"AbdiEngine-sc-lgEnd": isLinearGradient && canGoEnd,
				}),
				children: [
					h("div", {
						ref: scrollerRef,
						className: cn("AbdiEngine-sc-scroller", scrollerClassName, {
							"AbdiEngine-sc-snap": isSnap,
							"AbdiEngine-sc-maskGradient": isMask,
							"AbdiEngine-sc-wheelEnabled": wheelScrollEnabled,
							"AbdiEngine-sc-maskStart": isMask && canGoStart,
							"AbdiEngine-sc-maskEnd": isMask && canGoEnd,
						}),
						...
						children: h("div", {
							...
							className: cn(scrollContentClassName, {
								"AbdiEngine-sc-snapStart": scrollSnapAlign === "start",
								"AbdiEngine-sc-snapCenter": scrollSnapAlign === "center",
							}),
							children,
						}),
					}),
					showButtons !== SHOW_BUTTONS.NEVER &&
						hsf("div", {
							className: cn("AbdiEngine-sc-carousel", chevronsClassName, {
								"AbdiEngine-sc-onHover": showButtons === SHOW_BUTTONS.ON_HOVER,
							}),
	...
	Object.assign(AbdiEngine, {
		...
		Player: {
			...AbdiEngine.Player,
			get origin() {
				return AbdiEngine.Platform?.PlayerAPI;
			},
		},
		GraphQL: {
			...AbdiEngine.GraphQL,
			get Request() {
				return AbdiEngine.Platform?.GraphQLLoader || AbdiEngine.GraphQL.Handler?.(AbdiEngine.GraphQL.Context);
			},
	...
	if (!AbdiEngine.ContextMenuV2._context) AbdiEngine.ContextMenuV2._context = AbdiEngine.React.createContext({});
	if (!AbdiEngine.ReactComponent.Navigation)
		AbdiEngine.ReactComponent.Navigation = exportedMemoFRefs.find((m) => fnStr(m.type.render).includes("navigationalRoot"));
	...
	(function waitForChunks() {
		...
		if (listOfComponents.every((component) => component.split(".").reduce((o, k) => o?.[k], AbdiEngine.ReactComponent) !== undefined)) return;
		...
		AbdiEngine.ReactComponent.Slider = wrapProvider(functionModules.find((m) => fnStr(m).includes("progressBarRef")));
		AbdiEngine.ReactComponent.Toggle = functionModules.find((m) => fnStr(m).includes("onSelected") && fnStr(m).includes('type:"checkbox"'));
		// Object.assign(AbdiEngine.ReactComponent.Cards, Object.fromEntries(cards));

		// chunks
		const dropdownChunk = chunks.find(([, value]) => fnStr(value).includes("dropDown") && fnStr(value).includes("isSafari"));
		if (dropdownChunk) {
			AbdiEngine.ReactComponent.Dropdown =
		...
		if (toggleChunk && !AbdiEngine.ReactComponent.Toggle) {
			AbdiEngine.ReactComponent.Toggle = Object.values(require(toggleChunk[0]))[0].render;
		}

		if (!listOfComponents.every((component) => component.split(".").reduce((o, k) => o?.[k], AbdiEngine.ReactComponent) !== undefined)) {
			setTimeout(waitForChunks, 100);
			return;
		}

		if (AbdiEngine.ReactComponent.ScrollableContainer) setTimeout(refreshNavLinks, 100);
	})();

	(function waitForSnackbar() {
		if (!Object.keys(AbdiEngine.Snackbar).length) {
	...
	if (AbdiEngine.Locale) AbdiEngine.Locale._supportedLocales = cache.find((m) => typeof m?.ja === "string");

	Object.defineProperty(AbdiEngine, "Queue", {
		get() {
			return AbdiEngine.Player.origin?._queue?._state ?? AbdiEngine.Player.origin?._queue?._queue;
		},
	});
	...
	if (!AbdiEngine.ReactComponent?.ConfirmDialog && confirmDialogChunk) {
		AbdiEngine.ReactComponent.ConfirmDialog = Object.values(require(confirmDialogChunk[0])).find((m) => typeof m === "object");
	} else {
		AbdiEngine.ReactComponent.ConfirmDialog = functionModules.find(
	...
	if (contextMenuChunk) {
		AbdiEngine.ReactComponent.ContextMenu = Object.values(require(contextMenuChunk[0])).find((m) => typeof m === "function");
	}
	...
	if (playlistMenuChunk && !AbdiEngine.ReactComponent?.PlaylistMenu) {
		AbdiEngine.ReactComponent.PlaylistMenu = Object.values(require(playlistMenuChunk[0])).find(
	...
	if (infiniteQueryChunk) {
		AbdiEngine.ReactQuery.useInfiniteQuery = Object.values(require(infiniteQueryChunk[0])).find((m) => typeof m === "function");
	}

	if (AbdiEngine.Color) AbdiEngine.Color.CSSFormat = modules.find((m) => m?.RGBA);

	// Combine snackbar and notification
	(function bindShowNotification() {
		if (!AbdiEngine.Snackbar?.enqueueSnackbar && !AbdiEngine.showNotification) {
			setTimeout(bindShowNotification, 250);
			return;
		}

		if (AbdiEngine.Snackbar?.enqueueSnackbar) {
			AbdiEngine.showNotification = (message, isError, msTimeout) => {
				AbdiEngine.Snackbar.enqueueSnackbar(message, {
		...
		AbdiEngine.Snackbar.enqueueSnackbar = (message, { variant = "default", autoHideDuration } = {}) => {
			isError = variant === "error";
			AbdiEngine.showNotification(message, isError, autoHideDuration);
		};
	})();

	// Image color extractor
	(async function bindColorExtractor() {
		if (!AbdiEngine.GraphQL.Request) {
		...
		AbdiEngine.extractColorPreset = async (image) => {
			const analysis = await imageAnalysis(AbdiEngine.GraphQL.Request, image);
	...
	function wrapProvider(component) {
		...
		return (props) =>
			AbdiEngine.React.createElement(
				AbdiEngine.ReactComponent.RemoteConfigProvider,
				{ configuration: AbdiEngine.Platform.RemoteConfiguration },
				AbdiEngine.React.createElement(component, props)
			);
	}
	...
	(function waitForURI() {
		...
		// URI.Type
		AbdiEngine.URI.Type = URIModules.find((m) => m?.PLAYLIST_V2);

		// Parse functions
		AbdiEngine.URI.from = URIModules.find((m) => typeof m === "function" && m.toString().includes("allowedTypes"));
		AbdiEngine.URI.fromString = URIModules.find((m) => typeof m === "function" && m.toString().includes("Argument `uri`"));

		// createURI functions
		const createURIFunctions = URIModules.filter((m) => typeof m === "function" && m.toString().match(/\([\w$]+\./));
		for (const type of Object.keys(AbdiEngine.URI.Type)) {
			...
			const camelCaseType = type
				...
				.join("");
			AbdiEngine.URI[`${camelCaseType}URI`] = func;
		}
		...
		for (const type of Object.keys(AbdiEngine.URI.Type)) {
			...
			AbdiEngine.URI[`is${camelCaseType}`] =
				func ??
				((uri) => {
					let uriObj;
					try {
						uriObj = AbdiEngine.URI.from?.(uri) ?? AbdiEngine.URI.fromString?.(uri);
					} catch {
						return false;
					}
					if (!uriObj) return false;
					return uriObj.type === AbdiEngine.URI.Type[type];
				});
		}

		AbdiEngine.URI.isPlaylistV1OrV2 = (uri) => AbdiEngine.URI.isPlaylist(uri) || AbdiEngine.URI.isPlaylistV2(uri);

		// Conversion functions
		AbdiEngine.URI.idToHex = URIModules.find((m) => typeof m === "function" && m.toString().includes("22==="));
		AbdiEngine.URI.hexToId = URIModules.find((m) => typeof m === "function" && m.toString().includes("32==="));

		// isSameIdentity
		AbdiEngine.URI.isSameIdentity = URIModules.find((m) => typeof m === "function" && m.toString().match(/[\w$]+\.id===[\w$]+\.id/));
	})();

	AbdiEngine.Events.webpackLoaded.fire();
	refreshNavLinks?.();
})();
...
// Wait for AbdiEngine.Player.origin._state before adding following APIs
(function waitOrigins() {
	if (!AbdiEngine?.Player?.origin?._state) {
	...
	const interval = setInterval(() => {
		if (!AbdiEngine.Player.origin._state?.item) return;
		AbdiEngine.Player.data = AbdiEngine.Player.origin._state;
		playerState.cache = AbdiEngine.Player.data;
		clearInterval(interval);
	}, 10);

	AbdiEngine.Player.origin._events.addListener("update", ({ data: playerEventData }) => {
		playerState.current = playerEventData.item ? playerEventData : null;
		AbdiEngine.Player.data = playerState.current;

		if (playerState.cache?.item?.uri !== playerState.current?.item?.uri) {
			const event = new Event("songchange");
			event.data = AbdiEngine.Player.data;
			AbdiEngine.Player.dispatchEvent(event);
		}
	...
	(function waitProductStateAPI() {
		if (!AbdiEngine.Platform?.UserAPI) {
			setTimeout(waitProductStateAPI, 100);
			return;
		}

		const productState = AbdiEngine.Platform.UserAPI._product_state || AbdiEngine.Platform.UserAPI._product_state_service;
		if (productState) return;
		if (!AbdiEngine.Platform?.ProductStateAPI) {
			setTimeout(waitProductStateAPI, 100);
			return;
		}

		const productStateApi = AbdiEngine.Platform.ProductStateAPI.productStateApi;
		AbdiEngine.Platform.UserAPI._product_state_service = productStateApi;
	})();

	(async function setButtonsHeight() {
		while (!AbdiEngine.CosmosAsync) {
			await new Promise((res) => setTimeout(res, 100));
		}
		const expFeatures = JSON.parse(localStorage.getItem("AbdiEngine-exp-features") || "{}");
		const isGlobalNavbar = expFeatures?.enableGlobalNavBar?.value;

		if (typeof isGlobalNavbar !== "undefined" && isGlobalNavbar === "control") {
			await AbdiEngine.CosmosAsync.post("sp://messages/v1/container/control", {
				type: "update_titlebar",
				height: AbdiEngine.Platform.PlatformData.os_name === "osx" ? "42" : "40",
			});
		}
	})();
	...
	setInterval(() => {
		if (playerState.cache?.isPaused === false) {
			...
			event.data = AbdiEngine.Player.getProgress();
			AbdiEngine.Player.dispatchEvent(event);
		}
	}, 100);

	AbdiEngine.addToQueue = (uri) => {
		return AbdiEngine.Player.origin._queue.addToQueue(uri);
	};
	AbdiEngine.removeFromQueue = (uri) => {
		return AbdiEngine.Player.origin._queue.removeFromQueue(uri);
	};
})();

AbdiEngine.getAudioData = async (uri) => {
	const providedURI = uri || AbdiEngine.Player.data.item.uri;
	const uriObj = AbdiEngine.URI.from?.(providedURI) ?? AbdiEngine.URI.fromString?.(providedURI);
	if (!uriObj || (uriObj.Type || uriObj.type) !== AbdiEngine.URI.Type.TRACK) {
		throw "URI is invalid.";
	}

	return await AbdiEngine.CosmosAsync.get(
		`https://spclient.wg.spotify.com/audio-attributes/v1/audio-analysis/${uriObj.getBase62Id?.() ?? uriObj.id}?format=json`
	);
};

AbdiEngine.colorExtractor = async (uri) => {
	const body = await AbdiEngine.CosmosAsync.get(`https://spclient.wg.spotify.com/colorextractor/v1/extract-presets?uri=${uri}&format=json`);
...
AbdiEngine._getStyledClassName = (args, component) => {
	const includedKeys = [
		"role",
		"variant",
		"semanticColor",
...
(function waitMouseTrap() {
	...
	function formatKeys(keys) {
		...
		if (typeof keys === "object") {
			if (!keys.key || !Object.values(KEYS).includes(keys.key)) {
				throw `AbdiEngine.Keyboard.registerShortcut: Invalid key ${keys.key}`;
			}
			...
			throw `AbdiEngine.Keyboard.registerShortcut: Invalid keys ${keys}`;
		}
		return keystroke;
	}
	...
	AbdiEngine.Keyboard = {
		...
		registerShortcut: (keys, callback) => {
			AbdiEngine.Mousetrap.bind(formatKeys(keys), callback);
		},
		_deregisterShortcut: (keys) => {
			AbdiEngine.Mousetrap.unbind(formatKeys(keys));
		},
		changeShortcut: (keys, newKeys) => {
			if (!keys || !newKeys) throw "AbdiEngine.Keyboard.changeShortcut: Invalid keys";

			const callback = Object.keys(AbdiEngine.Mousetrap.trigger()._directMap).find((key) => key.startsWith(formatKeys(keys)));
			if (!callback) throw "AbdiEngine.Keyboard.changeShortcut: Shortcut not found";

			AbdiEngine.Keyboard.registerShortcut(newKeys, AbdiEngine.Mousetrap.trigger()._directMap[callback]);
			AbdiEngine.Keyboard._deregisterShortcut(keys);
		},
	};
	AbdiEngine.Keyboard.registerIsolatedShortcut = AbdiEngine.Keyboard.registerShortcut;
	AbdiEngine.Keyboard.registerImportantShortcut = AbdiEngine.Keyboard.registerShortcut;
	AbdiEngine.Keyboard.deregisterImportantShortcut = AbdiEngine.Keyboard._deregisterShortcut;
})();
...
(async function waitUserAPI() {
	...
	const productState =
		AbdiEngine.Platform.UserAPI?._product_state ||
		AbdiEngine.Platform.UserAPI?._product_state_service ||
		AbdiEngine.Platform?.ProductStateAPI.productStateApi;
...
function parseIcon(icon, size = 16) {
	if (icon && AbdiEngine.SVGIcons[icon]) {
		return `<svg height="${size}" width="${size}" viewBox="0 0 ${size} ${size}" fill="currentColor">${AbdiEngine.SVGIcons[icon]}</svg>`;
	}
	return icon || "";
}

function createIconComponent(icon, size = 16) {
	return AbdiEngine.React.createElement(
		AbdiEngine.ReactComponent.IconComponent,
		{
			iconSize: size,
...
AbdiEngine.ContextMenuV2 = (() => {
	...
	class Item {
		constructor({ children, disabled = false, leadingIcon, trailingIcon, divider, onClick, shouldAdd = () => true }) {
			...
			this._element = AbdiEngine.ReactJSX.jsx(() => {
				const [_children, setChildren] = AbdiEngine.React.useState(this._children);
				const [_disabled, setDisabled] = AbdiEngine.React.useState(this._disabled);
				const [_leadingIcon, setLeadingIcon] = AbdiEngine.React.useState(this._leadingIcon);
				const [_trailingIcon, setTrailingIcon] = AbdiEngine.React.useState(this._trailingIcon);
				const [_divider, setDivider] = AbdiEngine.React.useState(this._divider);

				AbdiEngine.React.useEffect(() => {
				...
				const context = AbdiEngine.React.useContext(AbdiEngine.ContextMenuV2._context) ?? {};

				return AbdiEngine.React.createElement(AbdiEngine.ReactComponent.MenuItem, {
					disabled: _disabled,
					divider: _divider,
					onClick: (e) => {
		...
		register() {
			AbdiEngine.ContextMenuV2.registerItem(this._element, this.shouldAdd);
		}
		deregister() {
			AbdiEngine.ContextMenuV2.unregisterItem(this._element);
		}
	}
	...
	class ItemSubMenu {
		...
		constructor({ text, disabled = false, leadingIcon, divider, items, depth = 1, shouldAdd = () => true }) {
			...
			this._depth = depth;
			this._element = AbdiEngine.ReactJSX.jsx(() => {
				const [_text, setText] = AbdiEngine.React.useState(this._text);
				const [_disabled, setDisabled] = AbdiEngine.React.useState(this._disabled);
				const [_leadingIcon, setLeadingIcon] = AbdiEngine.React.useState(this._leadingIcon);
				const [_divider, setDivider] = AbdiEngine.React.useState(this._divider);
				const [_items, setItems] = AbdiEngine.React.useState(this._items);
				const [_depth, setDepth] = AbdiEngine.React.useState(this._depth);
				...
				return AbdiEngine.React.createElement(AbdiEngine.ReactComponent.MenuSubMenuItem, {
					displayText: _text,
					divider: _divider,
					depth: _depth,
	...
	const renderItems = () => {
		const { props, trigger, target } = AbdiEngine.React.useContext(AbdiEngine.ContextMenuV2._context) ?? {};
...
AbdiEngine.Menu = (() => {
	...
	class Item extends AbdiEngine.ContextMenuV2.Item {
	...
	class SubMenu extends AbdiEngine.ContextMenuV2.ItemSubMenu {
...
AbdiEngine.ContextMenu = (() => {
	const iconList = Object.keys(AbdiEngine.SVGIcons);
	...
	class Item extends AbdiEngine.ContextMenuV2.Item {
		...
		constructor(name, onClick, shouldAdd = () => true, icon = undefined, trailingIcon = undefined, disabled = false) {
			super({
				...
				trailingIcon,
				onClick: (context) => {
					const [uris, uids, contextUri] = AbdiEngine.ContextMenuV2.parseProps(context.props);
					onClick(uris, uids, contextUri);
				},
				shouldAdd: (props) => {
					const parsedProps = AbdiEngine.ContextMenuV2.parseProps(props);
...
AbdiEngine._renderNavLinks = (list, isTouchScreenUi) => {
	const [refreshCount, refresh] = AbdiEngine.React.useReducer((x) => x + 1, 0);
	refreshNavLinks = refresh;

	if (
		!AbdiEngine.ReactComponent.ButtonTertiary ||
		!AbdiEngine.ReactComponent.Navigation ||
		!AbdiEngine.ReactComponent.TooltipWrapper ||
		!AbdiEngine.ReactComponent.ScrollableContainer ||
		!AbdiEngine.Platform.History ||
		!AbdiEngine.Platform.LocalStorageAPI
	)
		return;

	const navLinkFactory = isTouchScreenUi ? NavLinkGlobal : NavLinkSidebar;

	if (!navLinkFactoryCtx) navLinkFactoryCtx = AbdiEngine.React.createContext(null);
	const registered = [];
	...
	for (const app of list) {
		...
		try {
			...
			request.open("GET", `AbdiEngine-routes-${app}.json`, false);
			request.send(null);
		...
		if (typeof appProper === "object") {
			appProper = appProper[AbdiEngine.Locale?.getLocale()] || appProper.en;
		}
		if (!appProper) {
	...
	(function addStyling() {
		if (document.querySelector("style.AbdiEngine-navlinks")) return;
		const style = document.createElement("style");
		style.className = "AbdiEngine-navlinks";
		style.innerHTML = `
	:root {
	...
	const wrapScrollableContainer = (element) =>
		AbdiEngine.React.createElement(
			...
			AbdiEngine.React.createElement(AbdiEngine.ReactComponent.ScrollableContainer, null, element)
		);
	...
	const NavLinks = () =>
		AbdiEngine.React.createElement(
			...
			registered.map((NavLinkElement) => AbdiEngine.React.createElement(NavLink, NavLinkElement, null))
		);
...
const NavLink = ({ appProper, appRoutePath, icon, activeIcon }) => {
	const isActive = AbdiEngine.Platform.History.location.pathname?.startsWith(appRoutePath);
	const createIcon = () => createIconComponent(isActive ? activeIcon : icon, 24);

	const NavLinkFactory = AbdiEngine.React.useContext(navLinkFactoryCtx);

	return NavLinkFactory && AbdiEngine.React.createElement(NavLinkFactory, { appProper, appRoutePath, createIcon, isActive }, null);
};

const NavLinkSidebar = ({ appProper, appRoutePath, createIcon, isActive }) => {
	const isSidebarCollapsed = AbdiEngine.Platform.LocalStorageAPI.getItem("ylx-sidebar-state") === 1;
		...
		AbdiEngine.React.createElement(
			...
			AbdiEngine.React.createElement(
				AbdiEngine.ReactComponent.Navigation,
				{
					to: appRoutePath,
					referrer: "other",
					className: AbdiEngine.classnames("link-subtle", "main-yourLibraryX-navLink", {
				...
				createIcon(),
				!isSidebarCollapsed && AbdiEngine.React.createElement(AbdiEngine.ReactComponent.TextComponent, { variant: "balladBold" }, appProper)
			)
		)
	);
};
...
const NavLinkGlobal = ({ appProper, appRoutePath, createIcon, isActive }) => {
	return AbdiEngine.React.createElement(
		AbdiEngine.ReactComponent.TooltipWrapper,
		{ label: appProper },
		AbdiEngine.React.createElement(AbdiEngine.ReactComponent.ButtonTertiary, {
			iconOnly: createIcon,
			className: AbdiEngine.classnames("link-subtle", "main-globalNav-navLink", "main-globalNav-link-icon", "custom-navlink", {
				"main-globalNav-navLinkActive": isActive,
			}),
			"aria-label": appProper,
			onClick: () => AbdiEngine.Platform.History.push(appRoutePath),
		})
	);
};
...
class _HTMLGenericModal extends HTMLElement {
	hide() {
		AbdiEngine.ReactDOM.unmountComponentAtNode(this.querySelector("main"));
		this.remove();
	}
...
<div class="GenericModal__overlay" style="z-index: 100;">
	<div class="GenericModal" tabindex="-1" role="dialog" aria-label="${title}" aria-modal="true">
		...
		if (AbdiEngine.React.isValidElement(content)) {
			AbdiEngine.ReactDOM.render(content, main);
...
AbdiEngine.PopupModal = new _HTMLGenericModal();

Object.defineProperty(AbdiEngine, "TippyProps", {
	value: {
		delay: [200, 0],
		animation: true,
		render(instance) {
...
AbdiEngine.Topbar = (() => {
	...
	class Button {
		constructor(label, icon, onClick, disabled = false, isRight = false) {
			...
			this.tippy = AbdiEngine.Tippy?.(this.element, {
				content: label,
				...AbdiEngine.TippyProps,
			});
			this.label = label;
		...
		set icon(input) {
			let newInput = input;
			if (newInput && AbdiEngine.SVGIcons[newInput]) {
				newInput = `<svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor">${AbdiEngine.SVGIcons[newInput]}</svg>`;
			}
			this._icon = newInput;
	...
	(function waitForPlatform() {
		if (!AbdiEngine.Platform?.History) {
			setTimeout(waitForPlatform, 100);
			return;
		}

		AbdiEngine.Platform.History.listen(() => waitForTopbarMounted());
	})();
...
AbdiEngine.Playbar = (() => {
	...
	class Button {
		...
		set icon(input) {
			let newInput = input;
			if (newInput && AbdiEngine.SVGIcons[newInput]) {
				newInput = `<svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor" stroke="currentColor">${AbdiEngine.SVGIcons[newInput]}</svg>`;
			}
			this._icon = newInput;
...
(async function checkForUpdate() {
	if (!AbdiEngine.Config) {
		setTimeout(checkForUpdate, 300);
		return;
	}
	const { check_AbdiEngine_update, version } = AbdiEngine.Config;
	// Skip checking if upgrade check is disabled, or version is Dev/version is not set
	if (!check_AbdiEngine_update || !version || version === "Dev") return;
	// Fetch latest version from GitHub
	try {
		let changelog;
		const res = await fetch("https://api.github.com/repos/AbdiEngine/cli/releases/latest");
		...
		if (semver !== version) {
			...
			content.id = "AbdiEngine-update";
			content.innerHTML = `
				<style>
					#AbdiEngine-update a {
						text-decoration: underline;
					}
					#AbdiEngine-update pre {
						cursor: pointer;
						font-size: 1rem;
						padding: 0.5rem;
					...
					#AbdiEngine-update hr {
						border-color: var(--spice-subtext);
						margin-top: 1rem;
						margin-bottom: 1rem;
					}
					#AbdiEngine-update ul,
					#AbdiEngine-update ol {
						padding-left: 1.5rem;
					}
					#AbdiEngine-update li {
						margin-top: 0.5rem;
						margin-bottom: 0.5rem;
						list-style-type: disc;
					}
					#AbdiEngine-update ol > li {
						list-style-type: decimal;
					}
					.AbdiEngine-update-space {
						margin-bottom: 25px;
					}
					.AbdiEngine-update-little-space {
						margin-bottom: 8px;
					}
				</style>
				<p class="AbdiEngine-update-space">Update AbdiEngine to receive new features and bug fixes.</p>
				...
				<ol>
					<li>Update AbdiEngine CLI</li>
					<pre class="AbdiEngine-update-little-space">AbdiEngine update</pre>
					<p>AbdiEngine will automatically apply changes to Spotify after upgrading to the latest version.</p>
					<p>If you installed AbdiEngine via a package manager, update using said package manager.</p>
				</ol>
			`;
			...
			(function waitForTippy() {
				...
				const tippy = AbdiEngine.Tippy(content.querySelectorAll("pre"), {
				...
				for (const instance of tippy) {
					instance.reference.addEventListener("click", () => {
						AbdiEngine.Platform.ClipboardAPI.copy(instance.reference.textContent);
			...
			const updateModal = {
				title: "Update AbdiEngine",
				content,
				isLarge: true,
			};

			new AbdiEngine.Topbar.Button(
				"Update AbdiEngine",
				...
				() => AbdiEngine.PopupModal.display(updateModal)
			);
		}
	} catch (err) {
		console.error(err);
